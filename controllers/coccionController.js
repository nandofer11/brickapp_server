const coccionController = {
    ...
    // Función para iniciar la cocción y actualizar la hora_inicio y humeada
    iniciarCoccion: (req, res) => {
        const { id } = req.params;
        const hora_inicio = new Date().toISOString().slice(11, 19); // Hora actual (HH:MM:SS)
        const humeada = 1;

        const sql = `UPDATE coccion SET hora_inicio = ?, humeada = ? WHERE id_coccion = ?`;

        req.db.query(sql, [hora_inicio, humeada, id], (err, result) => {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
            res.status(200).json({ message: 'Cocción iniciada exitosamente' });
        });
    },
